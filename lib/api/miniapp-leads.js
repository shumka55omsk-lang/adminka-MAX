...
function normalizeLeadId(value) {
  ...
  // В старых версиях id мог быть числом, в текущей Supabase-схеме это UUID.
  // Для PostgREST безопасно передаём его как строку и не приводим к Number.
...
async function handleUpdate(req, res) {
  const id = normalizeLeadId(req.body?.id || req.query?.id || '');
  if (!id) {
...
async function handleDelete(req, res) {
  const id = normalizeLeadId(req.query?.id || req.body?.id || '');
  if (!id) {
    return res.status(400).json({ ok: false, error: 'Некорректный id заявки' });
  }

  const currentRows = await supabaseFetch(`max_miniapp_leads?select=id,name,phone,created_at&id=eq.${encodeURIComponent(id)}&limit=1`, {
    method: 'GET'
  });
  const current = Array.isArray(currentRows) ? currentRows[0] : null;
  if (!current) return res.status(404).json({ ok: false, error: 'Заявка не найдена или уже удалена' });

  const deletedRows = await supabaseFetch(`max_miniapp_leads?id=eq.${encodeURIComponent(id)}`, {
    method: 'DELETE',
    headers: { Prefer: 'return=representation' }
  });

  // Дополнительно пробуем убрать дубль из очереди max_crm_leads, если такая таблица/колонка есть.
  // Ошибка здесь не должна мешать удалению из раздела мини-приложения.
  let queueDeleted = 0;
  try {
    const queueRows = await supabaseFetch(`max_crm_leads?select=id&miniapp_lead_id=eq.${encodeURIComponent(id)}&limit=50`, { method: 'GET' });
    const queueIds = (Array.isArray(queueRows) ? queueRows : []).map((row) => row.id).filter(Boolean);
    if (queueIds.length) {
      await supabaseFetch(`max_crm_leads?id=in.(${queueIds.map((x) => encodeURIComponent(String(x))).join(',')})`, {
        method: 'DELETE',
        headers: { Prefer: 'return=minimal' }
      });
      queueDeleted = queueIds.length;
    }
  } catch {
    queueDeleted = 0;
  }
  ...
  return res.status(200).json({
    ok: true,
    deleted: Array.isArray(deletedRows) ? deletedRows.length : 1,
    lead: current,
    queueDeleted
  });
}
