...
export function normalizeButtons(input) {
  ...
  for (const raw of input) {
    ...
    const type = String(raw?.type || 'link').trim();
    if (type === 'open_app') {
      const webApp = String(raw?.web_app || raw?.webApp || '').trim();
      const payload = String(raw?.payload || '').trim().replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 256);
      if (/^[a-zA-Z0-9_]+_bot$/.test(webApp)) {
        safe.push({ type: 'open_app', text, web_app: webApp, payload });
      }
      continue;
    }
...
function buildAttachments(buttons, imagePayload) {
  ...
  if (buttons.length) {
    // MAX возвращает errors.maxRowSize, если в одной строке inline-клавиатуры слишком много кнопок.
    // Поэтому отправляем каждую ссылочную кнопку отдельной строкой: это безопаснее для MAX и удобно на телефоне.
    const keyboardRows = buttons.map((btn) => {
      if (btn.type === 'open_app') {
        ...
        if (btn.payload) button.payload = btn.payload;
        return [button];
      }
      return [{
        type: 'link',
