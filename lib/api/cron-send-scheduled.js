...
function isVercelCronRequest(req) {
  ...
  const cronScheduleHeader = String(req.headers['x-vercel-cron-schedule'] || '').trim();
  const userAgent = String(req.headers['user-agent'] || '').toLowerCase();
  return cronHeader === '1' || Boolean(cronScheduleHeader) || userAgent.includes('vercel-cron');
}
...
function isAuthorizedCron(req) {
  ...
  // Если CRON_SECRET не задан, разрешаем только штатный вызов Vercel Cron.
  // Это нужно, чтобы расписание работало сразу после добавления vercel.json.
