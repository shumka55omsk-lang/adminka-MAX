...
function normalizeLegacyWindow(w, index = 0) {
  const width = compactNumber(w?.widthCm ?? w?.width_cm ?? w?.width ?? w?.w_cm);
  const height = compactNumber(w?.heightCm ?? w?.height_cm ?? w?.height ?? w?.h_cm);
  const count = Math.max(1, compactNumber(w?.count ?? w?.qty ?? w?.quantity, 1));
  const area = Number(((width / 100) * (height / 100) * count).toFixed(3));
  return {
    id: index + 1,
    title: `Окно ${index + 1}`,
    name: `Окно ${index + 1}`,
    label: `Окно ${index + 1}: ${width}×${height} см × ${count}`,
    width_cm: width,
    height_cm: height,
    widthCm: width,
    heightCm: height,
    ...
    dimensions: `${width}×${height} см × ${count}`,
    size_text: `${width}×${height} см × ${count}`,
    shape: 'rectangle',
    type: 'window',
...
function buildLegacyItems(lead) {
  ...
  return (Array.isArray(f.windows) ? f.windows : [])
    .map((w, index) => normalizeLegacyWindow(w, index))
...
function buildLegacyTotals(lead, savedLeadId) {
  ...
  return {
    area_m2: compactNumber(estimate.areaM2),
    openings_count: compactNumber(estimate.openingsCount || items.reduce((sum, item) => sum + item.count, 0)),
    windows_count: items.reduce((sum, item) => sum + item.count, 0),
    ...
    estimated_total: Math.round(compactNumber(estimate.total)),
...
async function patchLegacyCalculationOptional(calculationId, patch) {
  if (!calculationId || !patch || !Object.keys(patch).length) return { skipped: true };
  try {
    ...
    return { ok: true, columns: Object.keys(patch) };
  } catch (error) {
    if (isMissingColumnError(error)) return { skipped: true, reason: error.message, columns: Object.keys(patch) };
    return { ok: false, error: error.message, columns: Object.keys(patch) };
  }
}
...
async function insertLegacyCalculation({ lead, savedLeadId, clientId }) {
  ...
  const row = {
    client_id: clientId,
    prices: buildLegacyPrices(lead),
    items,
    totals: buildLegacyTotals(lead, savedLeadId),
    materials: buildLegacyMaterials(lead)
  };
  ...
  optionalPatches.push(await patchLegacyCalculationOptional(id, { data: { ...row, windows: items, sizes_text: sizesText, raw_lead: lead } }));
  optionalPatches.push(await patchLegacyCalculationOptional(id, { calculation_data: { ...row, windows: items, sizes_text: sizesText, raw_lead: lead } }));
  optionalPatches.push(await patchLegacyCalculationOptional(id, { raw_payload: { ...row, windows: items, sizes_text: sizesText, raw_lead: lead } }));

  return { id, data, sizes_text: sizesText, items_count: items.length, optional_patches: optionalPatches };
}
...
async function insertLegacyHistory({ lead, savedLeadId, clientId, calculationId }) {
  ...
  const text = [
    ...
    amount ? `Предварительно: ${amount.toLocaleString('ru-RU')} ₽` : ''
  ].filter(Boolean).join('\n');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const lead = {
      form: { leadMode, requestType, name, phone, address, objectType, mountingType, expectedDate, needZippers, comment, windows },
      phone,
      estimate: {
