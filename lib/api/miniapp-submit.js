...
function buildLegacyItems(lead) {
  const f = lead.form || {};
  return (Array.isArray(f.windows) ? f.windows : []).map((w, index) => ({
    id: index + 1,
    title: `Окно ${index + 1}`,
    width_cm: compactNumber(w.widthCm),
    height_cm: compactNumber(w.heightCm),
    count: Math.max(1, compactNumber(w.count, 1)),
    area_m2: Number(((compactNumber(w.widthCm) / 100) * (compactNumber(w.heightCm) / 100) * Math.max(1, compactNumber(w.count, 1))).toFixed(3)),
    shape: 'rectangle',
    source: 'max_miniapp'
  }));
}
...
function buildLegacyTotals(lead, savedLeadId) {
  ...
  return {
    ...
    openings_count: compactNumber(estimate.openingsCount),
...
async function findOrCreateLegacyClient(lead) {
  ...
  if (phone) {
    try {
      const rows = await supabaseFetch(`clients?select=id,name,phone,address&phone=eq.${encodeURIComponent(phone)}&limit=1`, { method: 'GET' });
      if (Array.isArray(rows) && rows[0]?.id) {
        ...
        if (Object.keys(patch).length) {
          try {
            await supabaseFetch(`clients?id=eq.${encodeURIComponent(rows[0].id)}`, {
              method: 'PATCH',
              headers: {
      ...
      if (isMissingColumnError(error)) {
        const rows = await supabaseFetch(`clients?select=id&phone=eq.${encodeURIComponent(phone)}&limit=1`, { method: 'GET' });
  ...
  try {
    ...
    if (!isMissingColumnError(error)) throw error;
    const fallback = await supabaseFetch('clients', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Prefer: 'return=representation'
...
async function insertLegacyCalculation({ lead, savedLeadId, clientId }) {
  ...
  return { id: Array.isArray(data) && data[0] ? data[0].id : null, data };
}
...
async function insertLegacyHistory({ lead, savedLeadId, clientId, calculationId }) {
  ...
  try {
    const data = await supabaseFetch('client_history', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Prefer: 'return=representation'
...
async function tryCrmIntegration(lead, savedLeadId) {
  ...
  try {
    ...
    result.legacyCrm = { ok: false, error: error.message };
    result.ok = false;
  }
