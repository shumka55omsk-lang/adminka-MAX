...
async function insertLegacyCalculation({ lead, savedLeadId, clientId }) {
  ...
  const row = {
    client_id: clientId,
    status: 'new',
    title: `MAX mini app — ${totals.total.toLocaleString('ru-RU')} ₽`,
    name: lead.form?.name || null,
    phone: lead.phone || lead.form?.phone || null,
    address: lead.form?.address || null,
    object_type: lead.form?.objectType || null,

    // ВАЖНО: раньше рабочая связка сайта/CRM показывала сумму на дашборде потому,
    // что сумма была в верхнем поле расчёта, а не только в client_history.
    // Поэтому v46 кладёт сумму сразу во все известные варианты полей расчёта.
    area_m2: totals.area_m2,
    area: totals.area_m2,
    total_area_m2: totals.area_m2,
    total: totals.total,
    ...
    deal_total: totals.total,
    deal_amount: totals.total,
    material_total: totals.material_total,
    production_total: totals.production_total,
    install_total: totals.install_total,
    installation_total: totals.installation_total,
    mounting_type: totals.mounting_type,
    need_install: totals.need_install,
    opening_type: totals.opening_type,
    product_variant: totals.product_variant,
    source: 'max_miniapp',
    miniapp_lead_id: savedLeadId || null,
    ...
    state: crmState,
    calc_state: crmState,
    calculation_data: crmState,
    raw_payload: crmState,
    comment: `Заявка из MAX mini app. Расчёт: ${totals.total.toLocaleString('ru-RU')} ₽\n${sizesText}`.trim(),
    note: `Заявка из MAX mini app. Расчёт: ${totals.total.toLocaleString('ru-RU')} ₽\n${sizesText}`.trim()
  };

  const insertResult = await insertLegacyRowAdaptive('calculations', row, { requiredKeys: ['client_id'] });
  ...
  return { id, data, insert_result: insertResult, sizes_text: sizesText, items_count: items.length, total: totals.total, optional_patches: optionalPatches };
}
...
async function tryCrmIntegration(lead, savedLeadId) {
  ...
  try {
    ...
    const clientDashboard = await patchLegacyClientDashboardOptional({ lead, savedLeadId, clientId: client.id, calculationId: calculation.id });
