...
function buildLegacyTotals(lead, savedLeadId) {
  ...
  return {
    ...
    product_variant: f.openingType || null,
...
async function insertLegacyHistory({ lead, savedLeadId, clientId, calculationId }) {
  ...
  const text = [
    ...
    f.openingType ? `Тип изделий: ${f.openingType}` : '',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const lead = {
      form: { leadMode, requestType, name, phone, address, objectType, mountingType, openingType, expectedDate, needZippers, comment, windows },
      phone,
      estimate: {
