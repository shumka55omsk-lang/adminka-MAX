...
function buildLegacyTotals(lead, savedLeadId) {
  ...
  items.forEach((x) => {
    ...
    const extra = edgeCost + compactNumber(x.straps, 0) * qty * compactNumber(pr.strap, 0) + zipperCost + installCost;
    const factor = x.open ? 1.6 : 1;
    const raw = base + extra;
    const markup = raw * (factor - 1);
    const row = raw * factor;

    native.totalQty += qty;
    native.totalArea += area;
    native.baseTotal += base;
    native.extraTotal += extra + markup;
    native.grandTotal += row;
    native.rows.push({ area, rowTotal: row, markup, factor, edgeCoef });
  });
