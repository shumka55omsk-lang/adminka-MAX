...
function buildLeadText(lead, savedId = null) {
  ...
  if (estimate.total) lines.push(`Расчёт по заявке: ${Number(estimate.total).toLocaleString('ru-RU')} ₽`);
  if (estimate.base || estimate.install || estimate.zippers || estimate.openingMarkup) {
    ...
    if (estimate.zippers) lines.push(`Молнии: ${Number(estimate.zippers || 0).toLocaleString('ru-RU')} ₽`);
    if (estimate.openingMarkup) lines.push(`Коэффициент открывания ×${Number(estimate.openingCoefficient || 1.6).toLocaleString('ru-RU')} только на изготовление: +${Number(estimate.openingMarkup).toLocaleString('ru-RU')} ₽`);
  }
...
function buildLegacyOpeningFlags(value) {
  const openingType = normalizeOpeningType(value);
  const openingProduct = isOpeningProduct(openingType);
  const fixedProduct = !openingProduct;
  const mode = openingProduct ? 'opening' : 'fixed';

  // ВАЖНО для CRM мягких окон: дашборд считает сумму только когда в строке
  // расчёта явно установлен тип изделия — «глухое» или «открывающееся».
  // В разных версиях CRM этот чекбокс назывался по-разному, поэтому
  // передаём все совместимые алиасы на уровне каждого окна и итогов.
  return {
    opening_type: openingType,
    openingType,
    product_variant: openingType,
    variant: openingType,
    window_type: openingType,
    windowType: openingType,
    product_type_label: openingType,
    productTypeLabel: openingType,
    type_label: openingType,
    open_type: openingType,
    ...
    // Глухие / fixed aliases.
    g: fixedProduct,
    gluh: fixedProduct,
    gluhie: fixedProduct,
    gluhaya: fixedProduct,
    gluhoye: fixedProduct,
    gluhoe: fixedProduct,
    deaf: fixedProduct,
    isDeaf: fixedProduct,
    is_deaf: fixedProduct,
    deafWindow: fixedProduct,
    deaf_window: fixedProduct,
    blind: fixedProduct,
    isBlind: fixedProduct,
    is_blind: fixedProduct,
    blindWindow: fixedProduct,
    blind_window: fixedProduct,
    fixed: fixedProduct,
    isFixed: fixedProduct,
    is_fixed: fixedProduct,
    fixedWindow: fixedProduct,
    fixed_window: fixedProduct,
    closed: fixedProduct,
    isClosed: fixedProduct,
    is_closed: fixedProduct,
    closedWindow: fixedProduct,
    notOpenable: fixedProduct,
    not_openable: fixedProduct,
    noOpen: fixedProduct,
    no_open: fixedProduct,
    глухое: fixedProduct,
    глухие: fixedProduct,
    gluh_checked: fixedProduct,
    fixed_checked: fixedProduct,
    deaf_checked: fixedProduct,
    blind_checked: fixedProduct,

    // Открывающиеся / openable aliases.
    o: openingProduct,
    op: openingProduct,
    otkr: openingProduct,
    otkryv: openingProduct,
    otkriv: openingProduct,
    open: openingProduct,
    isOpen: openingProduct,
    is_open: openingProduct,
    opening: openingProduct,
    isOpening: openingProduct,
    is_opening: openingProduct,
    openable: openingProduct,
    isOpenable: openingProduct,
    is_openable: openingProduct,
    openingWindow: openingProduct,
    opening_window: openingProduct,
    openableWindow: openingProduct,
    openable_window: openingProduct,
    canOpen: openingProduct,
    can_open: openingProduct,
    otkrivayushiesya: openingProduct,
    otkryvayushiesya: openingProduct,
    открывающееся: openingProduct,
    открывающиеся: openingProduct,
    open_checked: openingProduct,
    opening_checked: openingProduct,
    openable_checked: openingProduct
  };
}
...
function openingCoefficientForType(value) {
  // В старой CRM галочка «Открывающиеся» включает коэффициент 1.6.
  // Бизнес-правило v53: коэффициент применяется только к изготовлению,
  // монтаж добавляется отдельно без умножения.
  return isOpeningProduct(value) ? 1.6 : 1;
}
...
function calculateServerEstimate(form = {}, rawEstimate = {}) {
  ...
  const openingType = normalizeOpeningType(form.openingType);
  const openingCoefficient = openingCoefficientForType(openingType);
  ...
  // Коэффициент «Открывающиеся» относится только к изготовлению.
  // Монтаж НЕ умножаем: итог = изготовление × 1.6 + монтаж.
  const manufacturingBeforeOpening = base + zippers;
  const subtotalBeforeOpening = manufacturingBeforeOpening + install;
  const openingMarkup = manufacturingBeforeOpening * (openingCoefficient - 1);
  const subtotal = manufacturingBeforeOpening + openingMarkup + install;
  ...
  return {
    ...
    manufacturingBeforeOpening: Math.round(manufacturingBeforeOpening),
    subtotalBeforeOpening: Math.round(subtotalBeforeOpening),
    openingCoefficient,
    openingMarkup: Math.round(openingMarkup),
...
function normalizeLegacyWindow(w, index = 0, lead = {}) {
  ...
  const itemManufacturingBeforeOpening = itemMaterialTotal;
  const itemSubtotalBeforeOpening = itemManufacturingBeforeOpening + itemInstallTotal;
  const itemOpeningCoefficient = openingCoefficientForType(openingType);
  const itemOpeningMarkup = Math.round(itemManufacturingBeforeOpening * (itemOpeningCoefficient - 1));
  const itemTotal = Math.round(itemManufacturingBeforeOpening + itemOpeningMarkup + itemInstallTotal);
  ...
  const openingFlags = buildLegacyOpeningFlags(openingType);
  ...
  return {
    ...
    // Точные ключи строки ручного калькулятора CRM. Если их нет, CRM
    // загружает строку как неполную и не фиксирует сумму на дашборде.
    ed: true,
    edging: true,
    edge: true,
    border: true,
    ...
    zc: 0,
    zd: 0,
    inst: isInstallSelected(f, lead.estimate || {}),
    install: isInstallSelected(f, lead.estimate || {}),
    door: false,
    isDoor: false,
    grommets: openingProduct,
    openingOnGrommets: openingProduct,
    ...
    manufacturing_before_opening: itemManufacturingBeforeOpening,
    subtotal_before_opening: itemSubtotalBeforeOpening,
    opening_coefficient: itemOpeningCoefficient,
    opening_markup: itemOpeningMarkup,
    total: itemTotal,
    sum: itemTotal,
    price: itemTotal,
...
function buildLegacyPrices(lead) {
  ...
  const openingFlags = buildLegacyOpeningFlags(f.openingType);
  return {
    // Точные ключи настроек старой CRM: loadCalc читает именно m2/edge/strap/zip/inst/off.
    m2: compactNumber(estimate.pricePerM2),
    edge: compactNumber(estimate.edgePrice, 0),
    strap: compactNumber(estimate.strapPrice, 0),
    zip: compactNumber(estimate.zipperPerOpening),
    inst: compactNumber(estimate.installPerM2),
    off: compactNumber(estimate.offset, 4.5),
    scraps: false,
    ...openingFlags,
    ...
    opening_coefficient: compactNumber(estimate.openingCoefficient, openingCoefficientForType(f.openingType)),
    openingCoefficient: compactNumber(estimate.openingCoefficient, openingCoefficientForType(f.openingType)),
    opening_markup: Math.round(compactNumber(estimate.openingMarkup, 0)),
    openingMarkup: Math.round(compactNumber(estimate.openingMarkup, 0)),
    manufacturing_before_opening: Math.round(compactNumber(estimate.manufacturingBeforeOpening, estimate.base)),
    subtotal_before_opening: Math.round(compactNumber(estimate.subtotalBeforeOpening, 0)),
...
function buildLegacyTotals(lead, savedLeadId) {
  ...
  const pr = buildLegacyPrices(lead);
  const openingFlags = buildLegacyOpeningFlags(f.openingType);

  // ВАЖНО: старая CRM фиксирует сумму на дашборде только после того, как
  // расчёт сохранён её родным калькулятором. Поэтому здесь повторяем
  // структуру и формулу функции totals() из CRM, как будто менеджер вручную
  // снял/поставил галочку «глухое/открывающееся» и расчёт автосохранился.
  const native = {
    totalQty: 0,
    totalArea: 0,
    baseTotal: 0,
  ...
  items.forEach((x) => {
    const qty = Math.max(1, compactNumber(x.qty || x.count || 1, 1));
    const wM = compactNumber(x.w || x.width || x.width_cm) / 100;
    const h1M = compactNumber(x.h1 || x.height1 || x.height1_cm || x.height) / 100;
    const h2M = compactNumber(x.h2 || x.height2 || x.height2_cm || x.height) / 100;
    const mhM = Math.max(h1M, h2M);
    const area = wM * mhM * qty;
    const base = area * compactNumber(pr.m2, 0);
    const per = wM + h1M + h2M + Math.sqrt((wM * wM) + ((h1M - h2M) ** 2));
    const hasZipper = Boolean(x.z);
    const zCount = hasZipper ? Math.max(1, compactNumber(x.zq || 2, 2)) : 0;
    const zipperCost = hasZipper ? (compactNumber(x.zc, 0) / 100) * zCount * qty * compactNumber(pr.zip, 0) : 0;
    const edgeCoef = hasZipper ? 2.5 : 1;
    const edgeCost = x.ed ? per * qty * compactNumber(pr.edge, 0) * edgeCoef : 0;
    const installCost = x.inst ? area * compactNumber(pr.inst, 0) : 0;
    const manufacturingExtra = edgeCost + compactNumber(x.straps, 0) * qty * compactNumber(pr.strap, 0) + zipperCost;
    const factor = x.open ? 1.6 : 1;
    const manufacturingRaw = base + manufacturingExtra;
    const markup = manufacturingRaw * (factor - 1);
    const row = manufacturingRaw + markup + installCost;

    native.totalQty += qty;
    native.totalArea += area;
    native.baseTotal += base;
    native.extraTotal += manufacturingExtra + installCost + markup;
    native.grandTotal += row;
    native.rows.push({ area, rowTotal: row, markup, factor, edgeCoef, installCost, manufacturingRaw });
  });

  const minOrder = compactNumber(pr.minOrder || pr.min_order || estimate.minOrder, 0);
  if (native.totalArea > 0 && minOrder > native.grandTotal) {
    const minOrderAdjustment = minOrder - native.grandTotal;
    native.extraTotal += minOrderAdjustment;
    native.grandTotal = minOrder;
    native.minOrderAdjustment = minOrderAdjustment;
  }

  const total = Math.round(native.grandTotal);
  const baseRounded = Math.round(native.baseTotal);
  const extraRounded = Math.round(native.extraTotal);
  const installTotal = items.reduce((sum, x) => sum + (x.inst ? (compactNumber(x.area_m2, 0) * compactNumber(pr.inst, 0)) : 0), 0);
  const windowsCount = native.totalQty;
  const areaM2 = Number(native.totalArea.toFixed(2));

  return {
    ...openingFlags,

    // Нативная структура CRM. Именно эти поля появляются после ручного
    // переключения галочки и нужны дашборду.
    totalQty: native.totalQty,
    totalArea: Number(native.totalArea.toFixed(3)),
    baseTotal: native.baseTotal,
    extraTotal: native.extraTotal,
    scrapArea: native.scrapArea,
    scrapCost: native.scrapCost,
    scrapCostInfo: native.scrapCostInfo,
    scrapPurchaseM2: native.scrapPurchaseM2,
    minOrderAdjustment: native.minOrderAdjustment || 0,
    rollArea: native.rollArea,
    cutArea: native.cutArea,
    grandTotal: native.grandTotal,
    rows: native.rows,
    ...
    count: windowsCount,
    base: baseRounded,
    material_total: baseRounded,
    install: Math.round(installTotal),
    install_total: Math.round(installTotal),
    zippers: Math.round(compactNumber(estimate.zippers)),
    zipper_total: Math.round(compactNumber(estimate.zippers)),
    extra: extraRounded,
    extra_total: extraRounded,
    opening_coefficient: compactNumber(estimate.openingCoefficient, openingCoefficientForType(f.openingType)),
    openingCoefficient: compactNumber(estimate.openingCoefficient, openingCoefficientForType(f.openingType)),
    opening_markup: Math.round(compactNumber(estimate.openingMarkup, Math.max(0, total - Math.round(compactNumber(estimate.subtotalBeforeOpening, 0))))),
    openingMarkup: Math.round(compactNumber(estimate.openingMarkup, Math.max(0, total - Math.round(compactNumber(estimate.subtotalBeforeOpening, 0))))),
    manufacturing_before_opening: Math.round(compactNumber(estimate.manufacturingBeforeOpening, baseRounded)),
    subtotal_before_opening: Math.round(compactNumber(estimate.subtotalBeforeOpening, total)),
    total,
    sum: total,
    total_sum: total,
    totalPrice: total,
    ...
    production_total: baseRounded,
    totalProduction: baseRounded,
    product_total: baseRounded,
    productTotal: baseRounded,
    installation_total: Math.round(installTotal),
    installTotal: Math.round(installTotal),
    montage_total: Math.round(installTotal),
    mounting_total: Math.round(installTotal),
    need_install: isInstallSelected(f, estimate),
    mounting_type: f.mountingType || null,
...
function buildLegacyMaterials(lead) {
  ...
  const items = buildLegacyItems(lead);
  return {
    // Нативные дефолты, чтобы отчёты/производство CRM не падали до ручного сохранения.
    rollRows: [],
    oversized: [],
    details: [],
    layouts: [],
    edgingRow: { material: 'Окантовка', width: '0 см', need: 0, rolls: 0, rest: 0, comment: 'Расход уточняется после проверки раскроя в CRM' },
    ...openingFlags,
    rows: items,
    items,
    mounting_type: f.mountingType || null,
...
async function touchLegacyClientAfterCalculation(clientId) {
  // Ручной saveCalc() в старой CRM после изменения галочки обновляет clients.updated_at.
  // Без этого дашборд может видеть расчёт, но не пересчитывать сумму до ручного действия.
  return patchLegacyClientOptional(clientId, { updated_at: new Date().toISOString() });
}

async function resaveLegacyCalculationNative(calculationId, row) {
  // Имитируем второй шаг ручного автосохранения CRM: после переключения
  // «глухое/открывающееся» CRM делает UPDATE существующего расчёта.
  // Поэтому после INSERT сразу делаем PATCH теми же нативными полями.
  if (!calculationId) return { skipped: true, reason: 'no_calculation_id' };
  try {
    await legacyFetch(`calculations?id=eq.${encodeURIComponent(calculationId)}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        Prefer: 'return=minimal'
    ...
    return { ok: true, table: 'calculations', mode: 'native_update_after_insert' };
  } catch (error) {
    if (isMissingColumnError(error) || isMissingTableError(error)) return { skipped: true, reason: error.message };
...
async function insertLegacyCalculation({ lead, savedLeadId, clientId }) {
  ...
  const openingFlags = buildLegacyOpeningFlags(lead.form?.openingType);
  const crmState = {
    ...openingFlags,
    source: 'max_miniapp',
    miniapp_lead_id: savedLeadId || null,
  ...
  // Сразу повторно сохраняем расчёт PATCH-ем в том же формате, как это делает
  // старая CRM при ручном переключении галочки «глухое/открывающееся».
  // Это важно для фиксации суммы на дашборде без ручного действия менеджера.
  const nativeResave = await resaveLegacyCalculationNative(id, row);
  const clientTouch = await touchLegacyClientAfterCalculation(clientId);
  ...
  return { id, data, sizes_text: sizesText, items_count: items.length, total: totals.total, native_resave: nativeResave, client_touch: clientTouch, optional_patches: optionalPatches };
}

async function insertLegacyHistory({ lead, savedLeadId, clientId, calculationId }) {
  const f = lead.form || {};
  const estimate = lead.estimate || {};
  const legacyTotals = buildLegacyTotals(lead, savedLeadId);
  const amount = Math.round(Number(legacyTotals.grandTotal || legacyTotals.total || estimate.total || 0));
