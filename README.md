# MAX Admin MVP v13 — закреплённый статус сверху

Что изменилось в v13:

- Верхний статус теперь `position: fixed`, всегда виден сверху экрана.
- Вверху виден бейдж `v13`, чтобы сразу понять, что обновление применилось.
- В заголовке страницы также указано `v13`.
- Добавлен `/api/version` для проверки версии деплоя.

Если после деплоя визуально ничего не изменилось:

1. Открой основной домен проекта, например `https://adminka-max.vercel.app`, а не старую preview-ссылку.
2. Сделай Vercel → Deployments → Redeploy.
3. Нажми Ctrl+F5 или открой сайт в инкогнито.
4. Проверь `https://adminka-max.vercel.app/api/version` — должно быть `version: v13-status-fixed`.
5. Если в GitHub загружаешь ZIP вручную, загружай содержимое архива в корень репозитория: папки `api`, `public`, `supabase` и файлы `vercel.json`, `package.json`, а не папку целиком внутрь старого проекта.
