...
## v9: исправление chat_id = 0

Если при отправке появляется ошибка `Invalid chatId: 0`, значит в таблице `max_groups` есть ошибочная строка с `chat_id = 0`. В v9 добавлена защита: такие группы не отправляются, ручное добавление `0` запрещено, а `supabase/schema.sql` удаляет такие строки и добавляет constraint.
